...
const AdminUniversities = () => {
	...
	// queries
	const { data: universities } = useUniversities(page, size)
	const { mutateAsync: createMutateAsync } = useCreateUnversity()
	const { mutateAsync: updateMutateAsync } = useUpdateUnversity()
	const { mutateAsync: deleteMutateAsync } = useDeleteUnversity()

	// pagenation
	const totalPages = universities?.totalPages ?? 0
	const handlePrev = () => {
	...
	return (
		<div>
			<div className='flex items-center justify-between py-4 px-4'>
				<h1 className='font-bold text-2xl'>Universitetlar</h1>
				...
				<TableFooter>
					<TableRow>
						<TableCell colSpan={8} className='text-right'>
							Jami universitetlar: {universities?.totalElements ?? 0}
						</TableCell>
					</TableRow>
				</TableFooter>
			</Table>
